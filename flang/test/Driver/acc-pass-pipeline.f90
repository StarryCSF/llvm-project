...
! RUN:   -mmlir --mlir-print-ir-before=acc-declare-action-conversion,acc-implicit-routine,fir-acc-recipe-bufferization,acc-recipe-materialization,acc-compute-lowering,convert-openacc-to-scf \
! RUN:   -o /dev/null %s 2>&1 | FileCheck %s --check-prefix=DEFAULT
! RUN: %flang_fc1 -fopenacc -emit-llvm -mmlir --disable-acc-pipeline \
! RUN:   -mmlir --mlir-print-ir-before=acc-initialize-fir-analyses,acc-declare-action-conversion,fir-acc-recipe-bufferization,acc-recipe-materialization,acc-compute-lowering,convert-openacc-to-scf \
