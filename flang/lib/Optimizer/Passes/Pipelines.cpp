...
void createDefaultFIROptimizerPassPipeline(mlir::PassManager &pm,
  ...
  // OpenACC pass block - runs before SCFToControlFlowPass so that SCF ops
  // generated by ACC passes (scf.if, scf.parallel, scf.for) are processed.
  pm.addPass(mlir::acc::createACCImplicitRoutine());
  pm.addPass(mlir::acc::createACCImplicitDeclare());
  pm.addPass(mlir::acc::createLegalizeDataValuesInRegion());
  pm.addPass(mlir::acc::createACCIfClauseLowering());
  pm.addPass(mlir::acc::createACCLegalizeSerial());
  pm.addPass(mlir::acc::createACCImplicitData());
  pm.addPass(fir::acc::createACCRecipeBufferizationPass());
  pm.addPass(mlir::acc::createACCRecipeMaterialization());
  pm.addPass(mlir::acc::createACCLoopTiling());
  pm.addPass(mlir::acc::createACCRoutineLowering());
  pm.addPass(mlir::acc::createACCSpecializeForHost());
  pm.addPass(mlir::acc::createACCSpecializeForDevice());
  pm.addPass(mlir::acc::createACCComputeLowering());
  pm.addPass(mlir::acc::createOffloadLiveInValueCanonicalization());
  pm.addPass(mlir::acc::createOffloadTargetVerifier());
  ...
  // This blocks HLFIR->FIR conversion from entering the region, so we must
  // re-run HLFIR->FIR lowering here to handle any remaining HLFIR ops inside
  // compute regions.
  pm.addPass(hlfir::createLowerHLFIROrderedAssignments(
      {/*tryFusingAssignments=*/false}));
